...
def attack1_loss(S_prime, mix_img, S, C, B, labels):
    ...
    # train_loss, train_loss_secret, attloss, loss_cover = \
    #       attack1_loss(recover_secret, mix_img, train_secrets, train_covers, beta, labels)
    ...
    classloss = torch.mean(getCEloss(labels, outputs))  # should be modified to the L2 distance between the extracted feature vectors of the secret image and the cover image
...
def attack2_loss(S_prime, mix_img, S, C, B, labels):
    ''' Calculates loss specified on the paper.'''
    # train_loss, train_loss_secret, attloss, loss_cover = \
    #       attack1_loss(recover_secret, mix_img, train_secrets, train_covers, beta, labels)
...
def getAdvZ(img_size,labels,batch_size,target_model,train=False,seed=None):
    c=10
    k=10
    ...
    for epoch in range(num_epochs):
        ...
        for idx, train_batch in tqdm(enumerate(train_loader)):         
            ...
            with torch.cuda.amp.autocast():
                train_covers, succ = getAdvZ(face_size, labels, len(train_secrets), target_model, train=True)
            cover_succ+= int(succ)
...
def test():
    MODELS_PATH = './output/Epoch_1_4999_output_checkpoint.pth.tar'  #../Weights/VGGFace2.pth.tar'
 
    
    net.eval()
    load_checkpoint(MODELS_PATH)
    outputname="jpdr_test"
    ...
    for idx, test_batch in enumerate(test_loader):
        ...
        test_covers, succ = getAdvZ(face_size, labels, test_batch_size, target_model, train=False)
        cover_succ += int(succ)
        print("att z succ rate:{}".format(cover_succ/total))
        
        mix_img, recover_secret = net(secret=test_secrets, cover=test_covers, train=False)
...
if __name__ == "__main__":
    ...
    train_data = build_dataset('/data/home/liuchunyu/code/uwf/dr_labeled_junior/train', input_size=face_size)
    train_num = len(train_data.samples)
    ...
    if stage == "test":
        test_data = build_dataset('/data/home/liuchunyu/code/uwf/dr_labeled_junior/test', input_size=face_size)
        test_num = len(test_data.samples)
        test_loader = torch.utils.data.DataLoader(
                test_data, batch_size=test_batch_size, num_workers=10, 
