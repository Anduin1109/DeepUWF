...
class MaskedAutoencoderViM(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 embed_dim=1024, depth=24, num_heads=16, drop_path=0.1,
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False,
                 flip_img_sequences_ratio=-1., **kwargs
                 ):
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, stride=patch_size, in_chans=in_chans, embed_dim=embed_dim
        )
        num_patches = self.patch_embed.num_patches
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path, decoder_depth)]  # stochastic depth decay rule
        inter_dpr = [0.0] + dpr
        ...
        self.decoder_norm = RMSNorm(decoder_embed_dim, eps=1e-5)
    ...
    def forward_encoder(self, x, mask_ratio, inference_params=None):
        ...
        return hidden_states, mask, ids_restore, None
...
class VisionMamba(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 embed_dim=1024, depth=24, drop_path=0.1, num_classes=1000,
    ...
    def forward_features(self, x, inference_params=None):
        ...
        if self.global_pool:
            ...
            return self.fc_norm(hidden_states)
...
def mae_mamba_large_patch16_dec512d8b(**kwargs):
...
def vim_large_patch16(**kwargs):
    model = VisionMamba(
        patch_size=16, in_chans=3,
        embed_dim=384, depth=24, norm_layer=partial(nn.LayerNorm, eps=1e-6), 
        **kwargs
    )
    # model = _VisionMamba(
    #     patch_size=16, stride=16, embed_dim=1024, depth=24, 
    #     rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True,
    #     **kwargs
    # )
...
mae_mamba_large_patch16 = mae_mamba_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
